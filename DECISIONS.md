...
## S1: REAL EARTH (2026-07-09) — ETOPO terrain + NASA textures + fast-mode plan

- **MASTER_PLAN de-calendared** (owner: "we will be moving fast") — dependency order only; Demo One ships when gate ④ lands.
- **Real Earth surface shipped (S1 Tier A):** global DEM baked from NOAA ETOPO 2022 60 arc-sec surface GeoTIFF (verified URL, 466 MB, public domain) → 8192×4096 Int16 LE real meters (land + bathymetry), gzip 49.9 MB at `public/data/`. New `demGrid.ts` (bilinear equirect sampler, antimeridian wrap, adaptive gzip loader — vite serves .gz with Content-Encoding so the loader sniffs the 1f8b magic). `createHeightField('earth', …, dem)` = DEM + sub-pixel procedural detail (ridged ±55 m fading below the 120 m shoreline). Workers get a one-time `{type:'dem'}` init; patch builds HOLD while the DEM streams (proxy stays up — covered=false). **Real vertical meters on the 0.1-radius globe is deliberate**: absolute altitudes couple correctly with the real-H atmosphere (Everest at true pressure altitude); relative slopes read 10× dramatic. Regression: 9 unit tests incl. real-asset spot checks (Himalaya > 5,000 m, Mariana < −8,000 m, Kansas plains, Southern-Ocean ring ≥ 90% sea level).
- **Earth textures replaced (S0.4 provenance fix):** Spline-extracted 4k set DELETED; NASA Blue Marble NG day (unshaded 200412 21600×10800 → 8k), Black Marble 2016 night → 8k, cloud_combined → 8k. All public domain; `LICENSES.md` manifest added.
