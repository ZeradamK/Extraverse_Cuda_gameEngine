...
- **Flight model** — 6DOF Newtonian with flight assist and **adaptive velocity**: the speed cap scales with distance to the nearest surface (Elite's d/4 rule) up to **4,000 mi/s** in open space, with 30,000 m/s² inertial dampers; Elite-style warp with line-of-sight obstruction checks and mass-lock emergency drop.
...
| Key | Action |
| --- | --- |
| `W` / `S` | thrust forward / **brake** — speed **adapts to open space** (250 m/s near the ground → thousands of km/s in the void, auto-braking on approach) |
| `W`+`SPACE` (hold) | **afterburner** — 5× the base speed with spool-up; release to decelerate back |
